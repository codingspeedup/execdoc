...
public final class BpCtx {

    public static final String ENCRYPTED_PROPERTY_SUFFIX = ".encrypted";

    private static BpCtxProvider envProvider = null;

    @Getter(lazy = true)
    private static final BpCtx instance = loadEnvironmentProperties();
    ...
    private BpCtx(BpCtxProvider envProvider) {
    ...
    public static void registerEnvironmentProvider(BpCtxProvider envProvider) {
        BpCtx.envProvider = envProvider;
    }

    private static BpCtx loadEnvironmentProperties() {
        if (envProvider == null) {
            System.err.println(BpCtx.class.getName() + ": no registered environment provider - quitting");
            System.exit(-1);
        }
        return new BpCtx(envProvider);
    }
